...
\begin{abstract}
Lightweight block ciphers target constrained IoT hardware where AES-128 exceeds area budgets,
yet their software and GPU throughput characteristics are rarely compared against AES on a single
controlled platform. This paper presents from-scratch C++ and CUDA implementations of PRESENT-80,
SPECK-64/128, and AES-128, each validated against official test vectors and benchmarked on an AMD EPYC
9374F processor with an NVIDIA L40S GPU. On the CPU, SPECK-64/128 sustains 1,132.77 MB/s in ECB
encryption at 64~MB, AES-128 reaches 23.87~MB/s, and PRESENT-80 reaches 1.4451~MB/s. On the GPU,
SPECK-64/128 peaks at 356,150~MB/s, AES-128 at 32,672~MB/s, and PRESENT-80 at 7,519~MB/s for ECB
encryption. GPU speedup over the CPU baseline is approximately 5,203 times for PRESENT-80, 1,384 times
for AES-128, and 314 times for SPECK-64/128. Counter mode (CTR) GPU throughput matches ECB for
PRESENT-80 and SPECK-64/128 but falls 37 percent for AES-128, identifying resource contention
specific to the AES table-driven design on the GPU. Key-schedule analysis shows SPECK-64/128 requires
only 100 cycles and 108 bytes of device constant memory, versus 8,465 cycles and 288 bytes for
PRESENT-80 and 360 cycles and 688 bytes for AES-128. The results demonstrate that ARX ciphers such as
SPECK simultaneously minimize silicon area and maximize software throughput, an alignment that
hardware-optimized SPN designs do not share.
\end{abstract}
...
All six mode roundtrip tests (ECB, CTR, and CBC for each cipher) pass complete encrypt-decrypt
verification. No timing data is recorded unless the corresponding test-vector check has passed.
...
GPU speedup over the CPU ECB baseline at 64~MB is approximately 5,203 times for PRESENT-80, 1,384
...
AES-128       & 32,671.82   & 35,436.36   & 20,626.12   & 20,638.33   & $\approx$1,384$\times$ \\
\hline
\end{tabular}
\end{center}
\end{table*}
...
\begin{figure*}[t]
\centerline{\includegraphics[width=\textwidth]{../run_outputs/speck64_performance_graphs.png}}
...
\begin{figure*}[t]
\centerline{\includegraphics[width=\textwidth]{../run_outputs/present80_performance_graphs.png}}
...
\begin{figure*}[t]
\centerline{\includegraphics[width=\textwidth]{../run_outputs/aes128_performance_graphs_corrected.png}}
